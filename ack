...
L<perlreref|http://perldoc.perl.org/perlreref.html>.  If you don't know
how to use regular expression but are interested in learning, you may
consult L<perlretut|http://perldoc.perl.org/perlretut.html>.  If you do not
...
Specifies an ackrc file to load after all others; see 
L</"ACKRC LOCATION SEMANTICS">.
...
B<--lines> options or as a comma separated list (B<--lines=3,5,7>). 
B<--lines=4-7> also works. The lines are always output in ascending order, no
matter the order given on the command line.
...
highlighting)
If PATTERN matches more than once then a line is output for each non-overlapping
match. For more information please see the section 
L</"Examples of F<--output>">.
...
B<--nopager> cancels any setting in ~/.ackrc, C<ACK_PAGER> or 
C<ACK_PAGER_COLOR>. No output will be sent through a pager.
...
files as well when searching for --perl files? 
I<ack --type-add perl:ext:xs --perl foo> does this for you. B<--type-add>
appends additional extensions to an existing type.
...
containing the word Bertrand use 
I<ack --type-set eiffel:ext:e,eiffel --eiffel Bertrand>.
As usual, you can also write B<--type=eiffel>
instead of B<--eiffel>. Negation also works, so B<--noeiffel> excludes
all eiffel files from a search. Redefining also works: 
I<ack --type-set cc:ext:c,h> and I<.xs> files no longer belong to the type
I<cc>.
...
from them.
So for example you can find a list of directories that do not get searched or
...
L<perlvar|http://perldoc.perl.org/perlvar.html#Variables-related-to-regular-expressions>.
...
MAIN:
{
    ...
    Getopt::Long::GetOptions(
        'help'       => sub { App::Ack::show_help(); exit 1; },
        'version'    => sub { App::Ack::print_version_statement(); exit 1; },
        'man'        => sub { App::Ack::show_man(); exit 1; },
    );
    Getopt::Long::Configure('default', 'no_auto_help', 'no_auto_version');
